...
// static char path_log[STR_LEN] = "SPT_new.log.txt";
...
int SPT_new::generateSPT_newfrom(int round, int node){
    ...
    for (int i=0;i<bottom;i++){
        ...
        if (dd[child]>0) {
        	//delta[v][j] means alpha(v, w) in the paper, where children[v][j] = w
...
double SPT_new::Build(int num, int bound)
{
    ...
    d[i] = max;
    old+=d[i];
//     FILE* f_log = fopen(path_log, "w");
//     fprintf(f_log, "selected: %d inc_inf:%f\n", mp, max);
